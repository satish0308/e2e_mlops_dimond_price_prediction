...
def fast_eda(df, target=None, correlation=True, pairplot=True, hist_box_plot=True, countplot=True):
    ...
    if df.shape > (1, 1):
        ...
        if correlation:
            ...
            if df_num.columns.nunique() < 10:    
                sns.heatmap(df_num.corr(), annot = True, cmap = "Spectral", linewidths = 2, linecolor = "#000000", fmt='.3f')
                plt.show()
            elif df_num.columns.nunique() < 15:    
                sns.heatmap(df_num.corr(), annot = True, cmap = "Spectral", linewidths = 2, linecolor = "#000000", fmt='.2f')
                plt.show()    
            elif df_num.columns.nunique() < 25:   
                sns.heatmap(df_num.corr(), annot = True, cmap = "Spectral", linewidths = 2, linecolor = "#000000", fmt='.1f')
                plt.show()
            else:
                sns.heatmap(df_num.corr(), annot = False, cmap = "Spectral")
                plt.show()
